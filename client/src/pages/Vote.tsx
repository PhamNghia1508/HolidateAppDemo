...
const choices = [
  ...
  { label: "Đổi giờ", icon: CalendarClock, variant: "danger" as const },
];
...
export default function Vote() {
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-20 px-5 relative" style={{ background: BG }}>
      ...
      {/* Plan Context Card — user always knows what they're voting on */}
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={S(1)}
        ...
        style={{ background: T1, boxShadow: "0 2px 12px rgba(15,23,42,0.18)" }}>
        <img
          src="https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=120&h=80&fit=crop"
          alt="Plan" className="w-[80px] h-[64px] object-cover flex-shrink-0"
        />
        <div className="py-2 pr-3 flex-1 min-w-0">
          <p className="text-[10px] font-bold uppercase tracking-widest text-white/50 mb-0.5">Đang vote</p>
          <p className="text-[15px] font-black text-white truncate">Rooftop chill night</p>
          <p className="text-[12px] text-white/60">Thứ bảy, 18:30 • 3 điểm đến</p>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={S(2)}
        ...
        <div className="progress-track">
          <motion.div
            style={{ background: `linear-gradient(90deg, ${BLUE_BRIGHT}, ${BLUE})`, height: "100%", borderRadius: 3 }}
            initial={{ width: "0%" }}
              ...
              <motion.button key={choice.label} whileHover={{ scale: 1.03, y: -2 }} whileTap={{ scale: 0.95 }}
                ...
                style={{
                  background: isSelected
                    ...
                    : choice.variant === "danger" ? "rgba(245,158,11,0.07)"
                    : "#F8FAFC"
                    : SURF2,
